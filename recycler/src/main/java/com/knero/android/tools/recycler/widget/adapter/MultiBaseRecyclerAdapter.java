...
public class MultiBaseRecyclerAdapter<T> extends RecyclerView.Adapter<MultiBaseRecyclerViewHolder<T>> {
    ...
    public MultiBaseRecyclerViewHolder<T> onCreateViewHolder(ViewGroup parent, int viewType) {
        MultiBaseRecyclerViewHolder<T> holder = mBaseViewHolderFactory.createViewHolder(viewType, LayoutInflater.from(mContext), parent);
    ...
    public void addItemsWithoutNotify(Collection<T> datas) {
        addItemsWithoutNotify(0, datas);
    }

    public void addItemsWithoutNotify(int type, Collection<T> datas) {
        for (T data : datas) {
            addItemWithoutNotify(type, data);
        }
    }

    public int addItemWithoutNotify(T data) {
        return addItemWithoutNotify(0, data);
    }

    public int addItemWithoutNotify(int type, T data) {
        mDatas.add(new BaseRecyclerItemInfo(type, data));
