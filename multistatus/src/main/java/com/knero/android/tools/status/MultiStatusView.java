package com.knero.android.tools.status;
...
public class MultiStatusView extends RelativeLayout {
    ...
    public void showError(int imgRes, final String error, String retryText, OnClickListener retryListener) {
        showError();
        ImageView imgView = mErrorView.findViewById(R.id.multi_status_error_icon);
        if (imgRes != -1) {
            imgView.setVisibility(View.VISIBLE);
            imgView.setImageResource(imgRes);
        }
        ...
        if (retryListener != null) {
            ...
            retryBtn.setOnTouchListener(new OnTouchListener() {
                @Override
                public boolean onTouch(View v, MotionEvent event) {
                    if (event.getAction() == MotionEvent.ACTION_DOWN) {
                        retryBtn.setTextColor(Color.WHITE);
                    } else {
                        retryBtn.setTextColor(getResources().getColor(R.color.text_color));
                    }
                    return false;
                }
            });
    ...
    public boolean canScrollVertically(int direction) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child != null && child.getVisibility() == View.VISIBLE) {
                return canScrollVertically(child, direction);
            }
        }
